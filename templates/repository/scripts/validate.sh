...
if not repo_yaml.exists():
    ...
    placeholders = {k: v for k, v in data.items() if isinstance(v, str) and v.startswith("{{")}
    if placeholders:
        errors.append(f"repository.yaml: placeholders sin rellenar {sorted(placeholders)}")
...
# TODO: añadir aquí validación específica del repositorio (lint de código, etc.)
