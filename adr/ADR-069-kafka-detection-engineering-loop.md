...
`CH-07` (replay de `Approval` tras reinicio del gateway) se confirmó `KNOWN_FAILING` en `ADR-068` (`ARG-020` sin cerrar: `ApprovalStore` en memoria de proceso). Consecuencia directa aquí: **mientras `CH-07` siga `KNOWN_FAILING`, ninguna aprobación SOC de una `RuleCandidate` puede desencadenar despliegue AUTOMÁTICO a Wazuh** — generar, validar, backtestear y presentar al SOC sí; aplicar tras la aprobación, no, hasta que exista persistencia durable de: `approval nonce`, `consumed state`, `action binding`, `expiration` (mismo defecto que R0-01/ARG-020, ahora también relevante para cambios de configuración defensiva, no solo acciones de contención). `services/rule_engineering.RuleDeploymentGate` implementa este bloqueo como código real, no como nota de prosa.
